...
export class Player {
    constructor(name, health, energy, isDefending, playStrategy, image, log) {
        this.name = name;
        this.health = health;
        this.energy = energy;
    ...
    getSprite() {
        ...
        const x = sprites[this.action] * 512;
...
export function createPlayers(log,random=false,numberOfPlayers=5) {
    if (random) {
        ...
        for (let i = 0; i < numberOfPlayers; i++) {
            players.push(new Player("Random "+ (i + 1), MAX_HEALTH, MAX_ENERGY, false, randomPlay,getSprite(i), log));
        }
        return players;
    }
    const players = [
        ...
        new Player("ChatGPT", MAX_HEALTH, MAX_ENERGY, false, chatGpt, "/sprites/magenta.png", log),
        //new Player("Gepeto", MAX_HEALTH, MAX_ENERGY, false, chatGpt, "/sprites/teal.png", log)
    ];

    return players;
}
export function createPlayer(name,playStrategy,image="/sprites/magenta.png",log) {
    return new Player(name, MAX_HEALTH, MAX_ENERGY, false, playStrategy, image, log);
}
